...
namespace WEA.Web
{
  ...
  */
        ...
        public static async Task AddUsers(IServiceProvider serviceProvider)
        {
            ...
            var user = await userManager.FindByEmailAsync("radjabrq@code.edu.az");
            if (user == null)
            {
                user = new User()
                {
                    Email = "radjabrq@code.edu.az",
                    UserName = "user",
                    IsAdmin = false,
                    EmailConfirmed = true,
                    PhoneNumberConfirmed = true
                };
                await userManager.CreateAsync(user, configuration["SeedAdminPW"]);
            }
        }
